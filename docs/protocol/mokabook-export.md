...
Mokabook owns artifact generation. Consumers own deployment, credentials,
...
npx mokabook export --config docs/mokabook.config.ts --out ../site --base main
```
...
must have a regular `.mokabook-export-artifact` ownership file using version 1
