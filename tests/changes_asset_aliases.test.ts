    ...
    test(`${kind} Changes follows a stable ${alias} alias to its edited target`, async (context) => {
      ...
      const source =
        kind === "screen"
          ...
          : validEntrySource() +
            `\nimport { definePage } from "mokly"; mockups.push(definePage({ id: "handbook", title: "Handbook", description: "Document", route: "handbook.html", dependencies: [], relatedDocs: [], render: () => '<html><body><img src="${route}" alt="Logo"/></body></html>' }));`;
