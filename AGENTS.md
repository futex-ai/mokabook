...
- Mockup HTML files under `docs/mockups/**/*.html` are generated by the
  published `mokabook` package. Canonical structured definitions live in
  `docs/mockups/src/entries/**/*.mockup.tsx`; shared TSX components live under
  `docs/mockups/src/components/` and `docs/mockups/src/pages/`. Exactly five
  intentional legacy `.source.tsx` entrypoints remain under
  `docs/mockups/src/pages/`, as documented in `docs/mockups/README.md` and
  enforced by `mockups:check`. Structured definitions and legacy entrypoints
  should compose TSX components, not large raw HTML strings or generated
  static-tree data. When changing reusable mockup source under
  `docs/mockups/src`, run `npm run mockups:build`, commit the matching generated
  HTML, run `npm run mockups:check`, `npm run mockups:test`, and
...
- Do not hand-edit generated mockup HTML as the source of truth. Update the
  structured definition, intentional legacy source, or shared component first,
  then let `mokabook` update the static HTML.
