...
export function installWorkspace(
  ...
  const tabs = installInspectorTabs(root, signal);
  ...
  const refresh = () => {
    ...
    if (controls && !selection.instance) controls.mount(panel("props")!);
    else {
      renderProps(panel("props")!, data, selection);
      if (controls) {
        const edit = element(doc, "button", "Edit component props");
        edit.type = "button";
    ...
    const reason = highlightUnavailable(
      root,
      views,
      frames,
      Boolean(variant.error || data.removed || variant.variant?.removed),
    );
    toggle.disabled = reason !== undefined;
    toggle.title = reason ?? "Highlight components";
    toggle.setAttribute(
      "aria-description",
    ...
    if (highlight && !reason)
      stopHighlight = installComponentHighlight(
        root,
        frames,
        selected,
        (key) => {
