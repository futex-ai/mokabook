...
export class ComponentControls {
  ...
  sync(
    ...
    const contextKey = contexts.map(key).join("|");
  ...
  views(
    ...
    return saved.map((view) => {
      const preview = this.previews.get(key(view));
      return preview
        ? {
            ...view,
  ...
  props() {
    ...
    return (
      (context ? this.previews.get(key(context))?.props : undefined) ??
      this.variant?.value.props
    );
  }
  ...
  private async verifyLoaded(
    ...
    const sequence = this.sequence;
    try {
      const expired = await componentPreviewExpired(
        frame,
